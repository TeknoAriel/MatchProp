...
El **buscador** usa el catálogo de propiedades en la base de datos. Ese catálogo se alimenta con **conexiones de ingest** (Yumblin, iCasas, Zonaprop, Externalsite, etc.) y se actualiza con un **cron horario** que recorre las conexiones activas.

---

## Conexiones activas (sin ejemplos en producción)

- Las **conexiones activas** se leen de **IngestSourceConfig** (tabla `IngestSourceConfig`, campo `sourcesJson`): cada clave con URL (ej. `yumblin`, `icasas`, `externalsite`) se considera activa.
...
- **Comparador / último id:** Cada fuente tiene un **SyncWatermark** (cursor por fuente). El conector devuelve `nextCursor` (ej. offset numérico para Yumblin/iCasas); ese valor se guarda y se envía en la siguiente ejecución para no repetir datos y avanzar.
