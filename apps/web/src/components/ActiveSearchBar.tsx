    ...
    <div className="sticky top-0 z-10 bg-[var(--mp-card)]/95 backdrop-blur border-b border-[var(--mp-border)]">
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-2.5">
        {!search ? (
          ...
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--mp-muted)] shrink-0"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
            <span className="text-sm text-[var(--mp-muted)]">Buscar propiedades…</span>
          </Link>
        ) : (
          <div className="flex items-center gap-3">
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-[var(--mp-foreground)] truncate" title={search.name}>
                {search.name}
              </p>
              <p className="text-xs text-[var(--mp-muted)] truncate" title={filtersToHumanSummary(search.filters)}>
